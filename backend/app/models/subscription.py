...
class SubscriptionPlan(Base):
    ...
    billing_cycle = Column(SAEnum(BillingCycle, name="billing_cycle", values_callable=lambda c: [e.value for e in c]), default=BillingCycle.MONTHLY, nullable=False)
...
class Subscription(Base):
    ...
    amount = Column(Float, nullable=False, default=0)
    billing_cycle = Column(SAEnum(BillingCycle, name="billing_cycle", values_callable=lambda c: [e.value for e in c]), default=BillingCycle.MONTHLY, nullable=False)
    status = Column(SAEnum(SubscriptionStatus, name="subscription_status", values_callable=lambda c: [e.value for e in c]), default=SubscriptionStatus.ACTIVE, nullable=False)
