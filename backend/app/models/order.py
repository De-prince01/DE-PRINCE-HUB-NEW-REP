...
class Order(Base):
    ...
    delivery_type = Column(SAEnum(DeliveryType, name="delivery_type"), default=DeliveryType.PICKUP)
...
class OrderStatusHistory(Base):
    ...
    id = Column(Uuid(as_uuid=True), primary_key=True, default=uuid.uuid4)
    order_id = Column(Uuid(as_uuid=True), ForeignKey("orders.id", ondelete="CASCADE"), nullable=False)
    from_status = Column(SAEnum(OrderStatus, name="order_status"))
    to_status = Column(SAEnum(OrderStatus, name="order_status"), nullable=False)
