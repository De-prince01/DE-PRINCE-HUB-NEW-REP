...
class Computer(Base):
    ...
    status = Column(SAEnum(ComputerStatus, name="computer_status", values_callable=lambda c: [e.value for e in c]), default=ComputerStatus.AVAILABLE, nullable=False)
    specs = Column(Text)
    notes = Column(Text)
    is_active = Column(Boolean, default=True, nullable=False)
    created_at = Column(DateTime(timezone=True), default=datetime.utcnow, nullable=False)
...
class Delivery(Base):
    ...
    delivery_type = Column(SAEnum(DeliveryType, name="delivery_type", values_callable=lambda c: [e.value for e in c]), nullable=False)
    address = Column(Text)
